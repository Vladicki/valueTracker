...
class SavedFoodsViewModelTest {
    ...
    @Test
    fun savedFoodsViewModel_loadsSavedFoodsAndSavedRecipes() = runTest {
        val repository = SavedFoodsFakeFoodRepository(
            savedFoods = listOf(Meal(167945, "Chicken Breast", 165, 31, 0, 4, "120 g", 0)),
            savedRecipes = listOf(Meal(1, "Chicken Breast + Garlic", 0, 0, 0, 0, "120 g", 0)),
        )
        val viewModel = SavedFoodsViewModel(repository)

        advanceUntilIdle()

        assertEquals(listOf("Chicken Breast"), viewModel.uiState.value.savedFoods.map { it.title })
        assertEquals(listOf("Chicken Breast + Garlic"), viewModel.uiState.value.savedRecipes.map { it.title })
    }

    @Test
    fun savedFoodsViewModel_filtersSavedFoodsAndRecipesByQuery() = runTest {
        val repository = SavedFoodsFakeFoodRepository(
            savedFoods = listOf(
            ...
            savedRecipes = listOf(
                Meal(1, "Chicken Breast + Garlic", 0, 0, 0, 0, "120 g", 0),
                Meal(2, "Berry Smoothie", 0, 0, 0, 0, "250 g", 0),
            ),
        )
        val viewModel = SavedFoodsViewModel(repository)
        ...
        assertEquals(listOf("Chicken Breast"), viewModel.uiState.value.filteredSavedFoods.map { it.title })
        assertEquals(listOf("Chicken Breast + Garlic"), viewModel.uiState.value.filteredSavedRecipes.map { it.title })
    }
}

private class SavedFoodsFakeFoodRepository(
    savedFoods: List<Meal> = emptyList(),
    savedRecipes: List<Meal> = emptyList(),
) : FoodRepository {
    private val savedFoodsFlow = MutableStateFlow(savedFoods)
    private val savedRecipesFlow = MutableStateFlow(savedRecipes)
    ...
    override fun observeSavedRecipes(): Flow<List<Meal>> = savedRecipesFlow.asStateFlow()
